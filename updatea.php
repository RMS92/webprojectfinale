...
{
     $requsermodif = $bdd->prepare("SELECT * FROM acheteur WHERE id_acheteur = ?");
     $requsermodif->execute(array($_SESSION['id_acheteur']));
     ...
     if(isset($_POST['newprenom']) AND !empty($_POST['newprenom']) AND $_POST['newprenom'] != $usermodif['prenom'])
     {
     	...
     	$insertprenom = $bdd->prepare("UPDATE acheteur SET prenom = ? WHERE id_acheteur = ?");
     	$insertprenom->execute(array($newprenom, $_SESSION['id_acheteur']));
      ...
      if(isset($_POST['newnom']) AND !empty($_POST['newnom']) AND $_POST['newnom'] != $usermodif['nom'])
     {
     	...
     	$insertnom = $bdd->prepare("UPDATE acheteur SET nom = ? WHERE id_acheteur = ?");
     	$insertnom->execute(array($newnom, $_SESSION['id_acheteur']));
     	header("Location: compteacheteur.php?id_acheteur=".$_SESSION['id_acheteur']);
     ...
     if(isset($_POST['newmail']) AND !empty($_POST['newmail']) AND $_POST['newmail'] != $usermodif['email'])
     {
     	...
     	if($mailexist == 0)
        {
     	    ...
     	    $insertmail = $bdd->prepare("UPDATE acheteur SET email = ? WHERE id_acheteur = ?");
     	    $insertmail->execute(array($newmail, $_SESSION['id_acheteur']));
     	    header("Location: compteacheteur?id_acheteur=".$_SESSION['id_acheteur']);
     	}
     	else
        {
     ...
     if(isset($_POST['newmdp']) AND !empty($_POST['newmdp']) AND isset($_POST['newmdp2']) AND !empty($_POST['newmdp2']))
     {
     	...
     	if($newmdp == $newmdp2)
     	{
     		$insertmdp = $bdd->prepare("UPDATE acheteur SET password = ? WHERE id_acheteur = ?");
     	    $insertmdp->execute(array($newmdp, $_SESSION['id_acheteur']));
     	    header("Location: compteacheteur.php?id_acheteur=".$_SESSION['id_acheteur']);
     	}
     	else
     	{
		...
		<div class="container-fluid">
			<div class="row">
				...
				<div class="col-lg-2 col-md-2 col-sm-12"> <!--height 80-->
				    ...
				    <a href="<?php echo "compteacheteur.php?id_acheteur=".$_SESSION['id_acheteur']."" ?>"><img class="petitlogo" src="images/compte.png" width="40px"></a>
						...
						<div class="bandeau_gauche">
							...
							<div class="formulaire" style="margin-top: 50px; margin-bottom: 25px">
						      ...
						      <br>
						       ...
						       <input type="submit" value="update informations" style="background-color: black; color: white;" /> 
						          </a>
