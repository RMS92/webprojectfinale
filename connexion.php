...
session_start();
   ...
   if(isset($_POST['inscriptionconnect']))
   {
   	    ...
   	    if(!empty($emailconnect) AND !empty($mdpconnect) AND (!empty($_POST['choixVconnect']) AND empty($_POST['choixAconnect'])) OR (empty($_POST['choixVconnect']) AND !empty($_POST['choixAconnect']))){

   	    	if(!empty($_POST['choixVconnect']) AND empty($_POST['choixAconnect']))
     		{
                /*VENDEUR*/
                $requser = $bdd->prepare("SELECT * FROM vendeur WHERE email = ? AND password = ?");
            	...
            	if($exist == 1)
            	{
            		...
            		$_SESSION['id_vendeur'] = $userinfosA['id_vendeur'];
            		$_SESSION['nom'] = $userinfosA['nom'];
            		$_SESSION['email'] = $userinfosA['email'];
            		header("Location: comptevendeur.php?id_vendeur=".$_SESSION['id_vendeur']);
            	}
            ...
            else if(empty($_POST['choixVconnect']) AND !empty($_POST['choixAconnect']))
            {/*ACHETEUR*/
            	$requser = $bdd->prepare("SELECT * FROM acheteur WHERE email = ? AND password = ?");
            	$requser->execute(array($emailconnect, $mdpconnect));
            	$exist = $requser->rowCount();
            	...
            	{
            		$userinfosA = $requser->fetch();
            		$_SESSION['id_acheteur'] = $userinfosA['id_acheteur'];
            		$_SESSION['nom'] = $userinfosA['nom'];
            		$_SESSION['email'] = $userinfosA['email'];
            		header("Location: compteacheteur.php?id_acheteur=".$_SESSION['id_acheteur']);
            	}
            	else 
            	{
            		$erreur = "Identifiants incorrects";
				...
				<div class="col-lg-7 col-md-7 col-sm-12" style="background-color: white;border:1px solid silver;">
		         ...
		         <form method="POST">
				      ...
				      <input type="password" id="password" name="mdpconnect" placeholder="Password">
				        ...
				        <label style="font-size: 25px">Type de compte: </label>
				      <br>
				      <label style="font-size: 25px; margin-top: 20px;"> Vendeur :</label>
							<input type="checkbox" name="choixVconnect" value="vendeur" style="font-size: 25px" />
							<label style="font-size: 25px"> Acheteur :</label>

							<input type="checkbox" name="choixAconnect" value="acheteur" />
				      
				      <input type="submit" value="Sign up" name="inscriptionconnect"/> 
				      <br>
				      <a href="inscription.php">
				      <input type="button" value=" Or Create an account" >
				      </a>
				 </form>
				 <?php
				if(isset($erreur)){
